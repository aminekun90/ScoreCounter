...
struct PlayerRow: View {
    ...
    var body: some View {
        HStack {
            ...
            Button(action: {
                deckController.updateScore(player.id, increment: false, amount:  Int64(incrementBy) ?? deckController.selectedDeck.increment)
            ...
            Button(action: {
                deckController.updateScore(player.id, increment: true, amount: Int64(incrementBy) ?? deckController.selectedDeck.increment)
                isPresentedWin = deckController.shouldWin(player: player)
            }) {
